...
local aa = {function()
	...
	return function(t)
		local u, v, w, x, y, z = e(k), {Minimized=false,Maximized=false,Size=t.Size,CurrentPos=0,Position=UDim2.fromOffset((j.ViewportSize.X / 2) - (t.Size.X.Offset / 2), (j.ViewportSize.Y / 2) - (t.Size.Y.Offset / 2))}, false;
		local A, B = false;
		local C = false;
	...
	c.New = function(d, e, f)
		...
		h.SetValue = function(m, n)
			...
			af:Create(l, TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {BackgroundTransparency=((h.Value and 0) or 1)}):Play();
	...
	ai.onStep = function(aj, c)
		return aj._onStep:connect(c);
	end;
	ai.onStart = function(aj, c)
		return aj._onStart:connect(c);
	end;
	ai.onComplete = function(aj, c)
		return aj._onComplete:connect(c);
	end;
	ai.start = function(aj)
	...
	return function()
		...
		it("should complete when all child motors are complete", function()
			...
			ai({A=ag.new(3),B=ah.new(4, {frequency=7.5,dampingRatio=1})});
			expect(ai._complete).to.equal(false);
			ai:step(0.01666666753590107);
			expect(ai._complete).to.equal(false);
			for aj = 1, 30 do
				ai:step(0.01666666753590107);
			end
	...
	ag.getValue = function(ah)
		return ah._state.value;
	end;
	ag.setGoal = function(ah, aj)
	...
	aj.new = function(c, d)
		...
		d = d or {};
		return setmetatable({_targetValue=c,_frequency=(d.frequency or 4),_dampingRatio=(d.dampingRatio or 1)}, aj);
	end;
	aj.step = function(c, d, e)
